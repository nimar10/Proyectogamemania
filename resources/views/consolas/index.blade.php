        ...
        <div class="row  mt-3">
            <nav class="navbar navbar-expand-lg navbar navbar-dark bg-dark" id="menu">
                ...
                <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
                    ...
                    <form class="form-inline my-2 my-lg-0" method="GET" action="{{ route('consolas.index') }}">
                        <select name="nombre" class='form-control' onchange="this.form.submit()">
                            <option value='%'>Nombre (Todos)</option>
                            @foreach ($cons as $consola)
                                @if ($consola == $request->nombre)
        ...
        <div class="row mt-1">
            <nav class="navbar navbar-dark bg-danger ">
                ...
                @can('Admin')
                    ...
                    <a href="{{ route('consolas.create') }}" class="btn btn-warning  float-right"
                        style="font-weight:bold;">Añadir Consola</a>
                @endcan
            </nav>
        </div>
                ...
                @foreach ($consolas as $item)
                    <div class="card ">
                        ...
                        @can('Admin')
                            ...
                            <form class="form-inline mb-2 ml-2" name="del" action="{{ route('consolas.destroy', $item) }}"
                                method="POST">
                                @method('DELETE')
                                @csrf
                                <a href="{{ route('consolas.edit', $item) }}" class="btn btn-warning mt-2 ml-2 mb-2 "><b>Opciones</b>
                                </a>
                                <input type="submit" onclick="return confirm('¿Borrar Consola?')"
                                            class="btn btn-danger mt-2 ml-2 mb-2" value="Eliminar">
